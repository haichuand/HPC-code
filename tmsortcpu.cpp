/*
  File: tmsortcpu.c
  Compile: g++ tmortcpu.c -o tmsortcpu -lpthread
  Use: tmsortcpu N
...
void allSort(int *x, int n) {
	...
	for (i=0; i<P-1; i++) {
		...
		if (pthread_create(&tPointer[i], &attr, tMergeSort, (void *)(intptr_t) i)) {
			printf("Error creating thread %d.\n", i);
			exit(0);
		}
	}
	CPU_ZERO(&cpus);
	CPU_SET((P-1)%numberOfProcessors, &cpus);
